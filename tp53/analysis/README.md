...
| `tp53_families.py` | Families by induced order (step 5) | over the 15 core-panel measures (proliferation screens held out), two families: 8 experimental readouts (within-r mean 0.78) vs 7 computational predictors (mean 0.76), **between-block mean 0.50** |
...
| `tp53_avg_rank.py` | Consensus poset + average-rank canonical (step 7) | over the 9 base measures (8 promoters + AlphaMissense), **56.6% incomparable**, field median-of-8 best proxy **ρ=0.987**, all 7 predictors ρ 0.46–0.60 (CADD 0.46 to AlphaMissense 0.60), canonical ClinVar AUC 0.993 |
...
| `tp53_reproducibility.py` | Constitutive vs artifactual, cross-**platform** (step 3) | yeast vs mammalian (the 3 Giacomelli conditions, Kotler not used here) consensus ρ 0.49 overall / **+0.63 extremes / −0.05 middle**, direction-agreement 0.53→0.85 |
...
(Frazer 2021, evemodel.org, P53_HUMAN); ESM1b (Brandes 2023, HuggingFace `ntranoslab/esm_variants`,
P04637); REVEL (Ioannidis 2016), CADD (Kircher 2014, v1.6 scores per Rentzsch 2021), PrimateAI
(Sundaram 2018) and BayesDel (Feng 2017), all via dbNSFP v4.1a (Liu 2020, Zenodo 4323592,
ENST00000269305 slice); ClinVar
